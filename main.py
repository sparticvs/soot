...
if repo_path is None:
    try:
        spec_repo = clone_repository(UPSTREAM_SPEC_REPO, SRPM_TMP_REPO,
            checkout_branch=UPSTREAM_SPEC_BRANCH)
...
if spec.version != latest_ver:
    ...
    spec_repo.checkout(f'update/v{latest_ver}')
    ...
    (commit, ref) = spec_repo.reslove_refish(f'update/v{latest_ver}')
    r = spec_origin.push(ref)
